...
export function AppStoreBadges({
  ...
  className?: string;
  /**
   * Below `sm`, drop the eyebrow ("Download on the" / "Get it on") and
   * tighten the padding so both badges share one row on a 344px screen:
   * 12 + 20 + 10 + label + 12 is ~120px and ~136px, 268px with the gap,
   * inside the 312px the strip has. The full badge (~150px each) could
   * not, and stacked. From `sm` up the badges are unchanged.
   */
  compact?: boolean;
}) {
  const badge =
    "inline-flex items-center gap-2.5 rounded-xl bg-forest-900 text-cream hover:bg-forest-800 transition-colors " +
    (compact ? "px-3 py-1.5 sm:px-4 sm:py-2" : "px-4 py-2");
  const eyebrow =
    "block text-[9px] uppercase tracking-wide opacity-80" +
    (compact ? " hidden sm:block" : "");
  const label =
    "block font-semibold " +
    (compact ? "text-sm/tight sm:text-[15px] sm:-mt-0.5" : "text-[15px] -mt-0.5");
  return (
    <div className={"flex flex-wrap items-center gap-3 " + className}>
      ...
      >
        ...
        <span className="leading-tight">
          <span className={eyebrow}>Download on the</span>
      ...
      >
        ...
        <span className="leading-tight">
          <span className={eyebrow}>Get it on</span>
