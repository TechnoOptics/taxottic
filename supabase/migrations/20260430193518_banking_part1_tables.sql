-- Recovered 20260430193518 (banking_part1_tables) from supabase_migrations.schema_migrations.
...
-- Locked-down secrets sibling. RLS enabled with NO policies so only
-- service-role (which bypasses RLS) ever reads or writes here. The
-- access_token never leaves server-side.
