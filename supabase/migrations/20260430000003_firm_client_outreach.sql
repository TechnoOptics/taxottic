...
as $$
  select
    o.id, o.firm_id, f.name, f.public_id, f.logo_url,
    o.full_name, o.business_name, o.tax_year, o.kind, o.message,
...
begin
  user_id_v := auth.uid();
  if user_id_v is null then raise exception 'auth_required'; end if;
  select email into user_email from auth.users where id = user_id_v;

  select * into o from public.firm_client_outreach
  where id = p_outreach_id and status = 'pending' and expires_at > now() limit 1;
  if not found then raise exception 'outreach_invalid'; end if;
  if lower(o.email) <> lower(user_email) then raise exception 'email_mismatch'; end if;

  select role into caller_role
  from public.company_members
  where user_id = user_id_v and company_id = p_company_id limit 1;
  ...
  insert into public.firm_engagements (
    ...
    requested_by, requested_by_side, responded_at, responded_by, client_note
  )
  values (
    o.firm_id, p_company_id, o.tax_year, o.kind, 'active',
    user_id_v, 'client', now(), user_id_v, o.message
  )
