-- Recovered 20260521072928 (push_notifications) from supabase_migrations.schema_migrations.
-- Applied out-of-band via the Supabase MCP/SQL editor and never
-- committed. Statements are verbatim from the history table, not
-- reconstructed from the schema. See docs/migration-history-state.md.

-- Push notifications backend. Repo migration 20260514000018 was never
-- applied to production; without device_tokens + notification_log,
-- every push registration silently no-ops. Idempotent (DO blocks +
-- if not exists) so applying on top of any partial state is safe.
