-- Extend the public invitation RPCs to surface the new manager-supplied
-- details (full name, title, personal welcome message) and copy them onto
-- the new member's profile / membership row when they accept.
--
-- DROP and recreate is required because we changed the OUT-row type, and
-- Postgres won't let you alter that with CREATE OR REPLACE.
