-- Audit 2026-07-25: the findings the verification agents never reached
-- (they died on a spend limit). Each was re-verified by hand against the
-- live catalog before this migration was written; one further finding
-- (an invitations policy said to subselect auth.users) was REFUTED - it
-- already uses auth.jwt() - so it is deliberately not touched here.
...
-- requires is_company_member (cf. "monthly_income: member insert").
...
create policy "mileage_trips driver full access" on public.mileage_trips
  ...
  with check (driver_user_id = auth.uid() and is_company_member(company_id));
...
-- U6 (major): the bank feeds missed the member-privacy scoping.
-- 20260704120000 deliberately ended "team transparency" for
-- monthly_income/monthly_expenses, but banking policies still let ANY
-- member read the company's whole transaction history and balances -
-- strictly more sensitive than the rows that were locked down.
...
create policy "bank_conn: manager read" on public.bank_connections
  for select using (is_company_manager(company_id) or is_super_admin());

-- U8 (minor): chat membership insert didn't constrain WHO gets added.
...
-- user from another company to a private group or DM and thereby grant
-- them full message read/write.
...
drop policy if exists "mileage_places firm read" on public.mileage_places;
create policy "mileage_places firm read" on public.mileage_places
  for select using (firm_has_active_engagement_with(company_id));

-- U7 (major, performance): no index served the render-from-raw window.
