...
final class TaxotticGeofenceStore {
    ...
    static void recordCapture(Context context, String state, String detail, int fixCount, long startedAtMs) {
        try {
            ...
            prefs(context)
                    ...
                    .putBoolean(KEY_CAPTURE_RUNNING, !CAPTURE_ENDED.equals(state))
                    .apply();
        } catch (JSONException e) {
    ...
    static boolean isCaptureRunning(Context context) {
        return prefs(context).getBoolean(KEY_CAPTURE_RUNNING, false);
    }
