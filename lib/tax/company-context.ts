...
export type CompanyRow = {
  ...
  // Manager receipt policy (item 10): cents above which an expense needs a
  // scanned receipt. null = no requirement.
  receipt_required_above_cents: number | null;
...
async function _loadCompanyByPublicId(publicId: string) {
  ...
  const { data: company } = await supabase
    .from("companies")
    .select(
      "id, public_id, name, entity_type, state_code, logo_url, receipt_required_above_cents, created_at",
    )
